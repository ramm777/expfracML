...
% Grid can be selected from a lot of pre-defined grids using the natural Camel mudrock. 
...
% ML (machine learning) data generation numerical 300 modesl
...
grid_type     = 'Carmel1a';              % GaussGrid2, Carmel1a, Carmel1b-JRCreduced
ubc =  repmat(5.34e-08, 1, 120); 
ubc(1) = ubc(1) / 10;            
totsteps = length(ubc);
...
save_.results    = 'standard';           % 'all' (inc Ghist), 'only_roughness', 'only_stressperm' 
save_.video      = false;
calc_.JRC        = false; 

if isunix; datapath = '/home/ak87/mrst-2017a/modules/vemmech/expfrac/Carmel/611No2DepthCurvesThin';  % Linux paths
else;      datapath = 'D:\mrst-2017a\modules\vemmech\expfrac\Carmel\611No2DepthCurvesThin';          % Windows path
end
shiftstep = 2; 
...
% Create Grid. Grid number is taken from the file (model) name   
x = mfilename;
i = uint16(str2double(x(5:end))); % Grid (curve item)
[G, ginfo] = createRandomSingleSectionsGrids(i, datapath, stepThin, delta, downsize, 'shiftstep', shiftstep); 
clear x i shiftstep downsize delta stepThin 
...
bc_el_sides{3}.el_bc.disp_bc.mask(:, 2) = true;
bc_el_sides{4} = bc{4}; % Upper
bc_el_sides{4}.el_bc.disp_bc.mask(:, 1) = false;
bc_el_sides{4}.el_bc.disp_bc.mask(:, 2) = true;
...
while s < totsteps
    ...
    %% CONTACT INTERACTION 
        ...
        % If contact is solved with no penetration => success   
            ...
            figure(4); clf; set(4, 'Position', [ss(1)*2310, ss(2)- 200, ss(3)/2, ss(4)/1.22]);
            plotCellData(G, stress(:, 1), demcells, 'FaceAlpha', 0.9, 'EdgeAlpha', 0.01); colorbar;
            title(['SOLVED Contact, Step= ', num2str(s-1)]); axis tight; % zoom(10); % colormap jet  
